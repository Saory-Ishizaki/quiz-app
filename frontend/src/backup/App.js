...
function normalizeQuestion(raw, fallbackPeso) {
  const perguntaText = raw.pergunta || raw.Pergunta;
  const opcoes = raw.opcoes || [raw.A, raw.B, raw.C, raw.D].filter(Boolean);
  const resposta = (raw.resposta || raw.Resposta || "").toString().trim();
    ...
    <div className="app-container">
      ...
      {stage === "playing" && questions.length > 0 && (
        <>
          <div className="status-bar">
            ...
            {LEVELS[nivelIndex].label} | <strong>Pontuação:</strong>{" "}
            {totalScore}
          </div>
      ...
      {stage === "levelResult" && (
        <LevelResult
          ...
          nextLabel={
            LEVELS[nivelIndex + 1] ? LEVELS[nivelIndex + 1].label : "Fim"
          }
